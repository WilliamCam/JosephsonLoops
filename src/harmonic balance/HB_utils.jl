...
#needs to be extended to a multidimensional case where its rotated for each variable 
...
function reconstruct_from_observed(h_prob::HarmonicProblem, var_name::String, order::Int, component::Symbol)
    ...
    #better documentation here and better variable names 
    #get refamiliarized with the code// ensure efficiency of the code and correctness of the code
    ...
    for (orig_var, (orig_name, deriv_order)) in orig_state_map
        ...
        for n in 1:N
            ...
            ddx_t += -An * nω^2 * cos(nω * t_sym) - Bn * nω^2 * sin(nω * t_sym) # rearrange the observed equations and substitute the ansatz in the state variables 
...
#one function that gives you the analytical form
#another one that applies that to the solution
